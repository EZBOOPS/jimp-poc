...
function extension.is_done()
    ...
    local talisman_stashed = not tracker.stash_talisman or (function()
        for _, item in pairs(get_local_player():get_talisman_items()) do
            if not utils.should_salvage_talisman(item) then return false end
        end
        return true
    end)()
    local result = (tracker.stash_count == 0) and
        ...
        talisman_stashed
    local sig = string.format('done=%s sc=%d sock=%s mat=%s sig=%s tal=%s flags{sock=%s boss=%s keys=%s sig=%s tal=%s}',
        tostring(result), tracker.stash_count,
        tostring(socketable_stashed), tostring(material_stashed), tostring(sigils_stashed), tostring(talisman_stashed),
        tostring(tracker.stash_socketables), tostring(tracker.stash_boss_materials),
        tostring(tracker.stash_keys), tostring(tracker.stash_sigils), tostring(tracker.stash_talisman))
...
task.shouldExecute = function ()
    ...
    if settings.skip_stash and not tracker.stash_done then
        tracker.stash_done = true
    end
