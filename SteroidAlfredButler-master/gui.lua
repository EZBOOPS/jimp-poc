...
gui.elements = {
    ...
    skip_stash = create_checkbox(false, 'skip_stash'),
    ...
    talisman_tree = tree_node:new(1),
    stash_talisman             = create_checkbox(false, 'stash_talisman'),
    talisman_salvage_rarity    = combo_box:new(0, get_hash(plugin_label .. '_talisman_salvage_rarity')),
    ...
    show_talisman_calibration  = create_checkbox(false, 'show_talisman_calibration'),
    show_inventory_calibration = create_checkbox(false, 'show_inventory_calibration'),
    inventory_slot_0_x    = slider_int:new(0, 3840, 1466, get_hash(plugin_label .. '_inv_slot_0_x')),
    inventory_slot_0_y    = slider_int:new(0, 2160, 700,  get_hash(plugin_label .. '_inv_slot_0_y')),
    inventory_cell_size_x = slider_int:new(20, 120, 67,   get_hash(plugin_label .. '_inv_cell_x')),
    inventory_cell_size_y = slider_int:new(20, 120, 67,   get_hash(plugin_label .. '_inv_cell_y')),
}
...
function gui.render()
    if not gui.elements.main_tree:push('[O] Alfred the Butler | Leoric | v' .. gui.plugin_version) then return end
    ...
    if gui.elements.general_tree:push('General settings') then
        ...
        gui.elements.skip_stash:render('Skip stash (override)', 'Disable all stashing — Alfred will salvage/sell but not visit the stash')
        if gui.elements.skip_stash:get() then
            render_menu_header('Stash is fully disabled. Items that would be stashed will stay in inventory.')
        end
    ...
    if gui.elements.talisman_tree:push('Talisman') then
        gui.elements.stash_talisman:render('Stash talisman on every town run', 'Uses get_talisman_items() API to move items to stash automatically')
        gui.elements.talisman_salvage_rarity:render('Salvage talisman up to rarity', {'Never', 'Magic', 'Rare', 'Legendary', 'Unique', 'Set'}, 'Salvages talismans at or below the selected rarity at the Occultist. Everything above is stashed.')
        gui.elements.talisman_tree:pop()
    end
