...
-- Talisman salvage rarity threshold: salvage talismans at or below the chosen rarity.
-- Rarity values: magic=1/2, rare=3/4, legendary=5, unique=6, set=7
utils.talisman_rarity_enum = {
    NEVER     = 0,
    MAGIC     = 1,
    RARE      = 2,
    LEGENDARY = 3,
...
local _talisman_rarity_thresholds = { [0]=-1, [1]=2, [2]=4, [3]=5, [4]=6, [5]=7 }
function utils.should_salvage_talisman(item)
    if not item then return false end
    local threshold = _talisman_rarity_thresholds[utils.settings and utils.settings.talisman_salvage_rarity or 0] or -1
...
local function get_uniques()
    local filename = get_plugin_root_path()
    filename = filename .. 'data\\affix\\unique-item.json'
    local file, err = io.open(filename,'r')
    if not file then
        utils.log('error opening file' .. filename)
        return
...
function utils.update_tracker_count(local_player)
    ...
    tracker.talisman_full = #get_local_player():get_talisman_items() >= 5
    ...
    tracker.need_trigger = tracker.inventory_full or
        ...
        tracker.need_gamble or
        tracker.talisman_full
